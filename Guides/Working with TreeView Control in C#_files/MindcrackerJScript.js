var max=5;var maxBrk=3;var check=-1;var checkBrk=-1;var onlyOnce=0;var maxBLeaders=5;var checkBLeaders=-1;var maxSponsors=5;var checkSponsors=-1;function ChangeByOne()
{if(document.getElementById("ctl00_ContentPlaceHolder1_FeaturedArticle1_hideNum")!=null)
max=parseInt(document.getElementById("ctl00_ContentPlaceHolder1_FeaturedArticle1_hideNum").value);if(check==-1)
setTimeout("forSetTimeOne()",1);else
setTimeout("forSetTimeOne()",30000);if(onlyOnce==0)
{ChangeByOneBrk();onlyOnce=1;}}
...
if(checkBrk==-1)
setTimeout("forSetTimeOneBrk()",1);else
setTimeout("forSetTimeOneBrk()",30000);}
function forSetTimeOne()
{check++;if(check==max)
check=0;HideShowOne(check);ChangeByOne();}
function forSetTimeOneBrk()
{checkBrk++;if(checkBrk==maxBrk)
checkBrk=0;HideShowOneBrk(checkBrk);ChangeByOneBrk();}
...
html+='\n</HEAD>\n<BODY>\n<form name = frmform>\n';var printReadyElement1=document.getElementById("div1");if(printReadyElement1!=null)
{html+=printReadyElement1.innerHTML;}
html+='\n</div>\n</form>\n</BODY>\n</HTML>';var sReturnString=document.getElementById("ctl00_ContentPlaceHolder1_txtNewsLetterContent");sReturnString.value=html;}
else
{}}
function CheckSession(sCheck)
{if(sCheck=="")
{var str=window.location.toString().toLowerCase();str=str.substring(str.indexOf('/c-sharpcorner/')+14);window.location.href('../UserRegistration/LoginCheck.aspx?ReturnURL=..'+str);return false;}
else
return true;}
function CheckSessionArticle(sCheck)
{if(sCheck=="")
{var str=window.location.toString().toLowerCase();window.location.href('/UserRegistration/LoginCheck.aspx?ReturnURL='+str);return false;}
else
return true;}
...
{alert("You are not authorized to "+sCommand+" the Comment");return false;}
else
...
{alert("You are not authorized to "+sCommand+" this Comment");return false;}
else
return confirm(sCommand+" the Message!");}
...
maxBLeaders=parseInt(document.getElementById("ctl00_ContentPlaceHolder3_WeeklyBlogLeaders1_hideNum").value);if(checkBLeaders==-1)
setTimeout("forSetTimeBlogLeaders()",1);else
setTimeout("forSetTimeBlogLeaders()",15000);}
function forSetTimeBlogLeaders()
{checkBLeaders++;if(checkBLeaders==maxBLeaders)
checkBLeaders=0;HideShowBlogLeaders(checkBLeaders);ChangeByBlogLeaders();}
...
maxBLeaders=parseInt(document.getElementById("ctl00_ContentPlaceHolder2_PhotoGallery1_hideNum").value);if(checkBLeaders==-1)
setTimeout("forSetTimeAuthorPhotoGallery()",1);else
setTimeout("forSetTimeAuthorPhotoGallery()",3500);}
function forSetTimeAuthorPhotoGallery()
{checkBLeaders++;if(checkBLeaders==maxBLeaders)
checkBLeaders=0;HideShowAuthorPhotoGallery(checkBLeaders);ChangeByAuthorPhotoGallery();}
